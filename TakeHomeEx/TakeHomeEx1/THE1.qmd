...
The easiest thing to fix first is the timestamp. Doing so will allow us to find the first and last points of a journey easily
...
tm_shape(osm_changi) + 
  ...
  tm_layout(legend.outside = TRUE, 
            main.title = 'TITLE TITLE HERE HERE' , main.title.size = 1)
...
all_maps.destination <- lapply(1:ncol(tnkde_densities.destination$k), function(i){
  ...
  tm_dots(col = "density", size = 0.01,
    ...
    tm_layout(legend.show=FALSE, main.title = paste("TNKDE CHANGI @", time, ":00"), main.title.size = 0.5)
  return(map1)
})
...
all_maps.destination <- lapply(1:ncol(tnkde_densities.destination$k), function(i){
  ...
  tm_dots(col = "density", size = 0.01,
          breaks = color_breaks$brks, palette = viridis(10)) + 
    tm_layout(legend.show=FALSE, main.title = paste("TNKDE WOODLANDS @", time, ":00"), main.title.size = 0.5)
  return(map1)
})
...
tmap_animation(all_maps.destination, filename = "images/tnkde_origin_woodlands.gif", 
               width = 1000, height = 1000, dpi = 300, delay = 50)
```
...
all_maps.destination <- lapply(1:ncol(tnkde_densities.destination$k), function(i){
  ...
  tm_dots(col = "density", size = 0.01,
          breaks = color_breaks$brks, palette = viridis(10)) + 
    tm_layout(legend.show=FALSE, main.title = paste("TNKDE DOWNTOWN CORE @", time, ":00"), main.title.size = 0.5)
  return(map1)
})
...
tmap_animation(all_maps.destination, filename = "images/tnkde_origin_town.gif", 
               width = 1000, height = 1000, dpi = 300, delay = 50)
```
